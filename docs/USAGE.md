...
当你只说“帮我学习”或“继续”时，Agent 会先检查当前课程状态，再用一个问题把你带到正确流程：
...
它只会询问还缺少的信息。如果你已经说“按考纲出 5 道 HTML 题”，就会直接进入出题流程，不再重复确认模式、题量和形态。

遇到这些状态时，Agent 会同时解释含义和下一步：

- **没有课程**：当前目录没有 `course.yaml`，需要先用 `init_course.py` 初始化课程工作区。
- **没有 KC**：还没有知识点骨架，建议先按考纲注册 KC，系统才能安排学习顺序。
- **没有题目**：需要注册真题/课后题，或明确请求生成迁移题；这不等于课程已经学完。
- **没有到期卡**：当前没有紧急 FSRS 复习任务，不代表所有知识点都已经掌握。
